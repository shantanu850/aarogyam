import 'dart:convert';
import 'package:aarogyam/Admin/UpdateOrder.dart';
...
class _PatientMyTestState extends State<Orders> {
  ...
  getData()async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await getId();
    http.Response rr = await http.post("http://www.notoutindia.com/aarogyam/api/GetOrders",
      headers: {
        "Authorization":prefs.getString('token'),
      },
    );
                                  ...
                                  child: Card(
                                    ...
                                    child: ListTile(
                                      trailing:PopupMenuButton(
                                        ...
                                        onSelected: (i) {
                                          if (i == 1) {
                                            Navigator.push(context, MaterialPageRoute(builder: (context)=>UpdateOrder(snapshot:snap.data[index],)));
                                          }
                                          if(i == 2){
                                            Navigator.push(context, MaterialPageRoute(builder: (context)=>UpdateOrderStatus(snapshot:snap.data[index],)));
                                          }
                                        },
                                      ),
