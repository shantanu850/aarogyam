...
import 'package:aarogyam/Admin/CreateTests.dart';
import 'package:aarogyam/Admin/UpdateTests.dart';
...
class Testss extends StatefulWidget {
  @override
  _TestssState createState() => _TestssState();
}
...
class _TestssState extends State<Testss> {
  ...
  getKeywords()async{
    SharedPreferences prefs = await SharedPreferences.getInstance();
    http.Response response = await http.get("http://www.notoutindia.com/aarogyam/api/GetTestRates",headers:{
      "Authorization":prefs.getString('token'),
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: Icon(Icons.search), onPressed: (){
            showSearch(context: context, delegate:SearchTest(searchList,0));
          })
        ],
      ),
      drawer: Drawer(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context)=>CreateTests()));
        },
                        ...
                        child: Padding(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Column(
                              ...
                              children: [
                                Text("  Normal Rate : ₹ ${snapshot.data[index]["NormalRate"]}"),
                                Text("  Lab Rate : ₹ ${snapshot.data[index]["LabRate"].toString().replaceAll("?", "")}"),
                              ],
                            ),
                            trailing:PopupMenuButton(
                              ...
                              onSelected: (i) {
                                if (i == 1) {
                                  Navigator.pushReplacement(context,MaterialPageRoute(builder: (context)=>UpdateTest(data:snapshot.data[index],)));
                                }
                              },
                            ),
                          ),
...
class SearchTest extends SearchDelegate{
  ...
  final type;
  SearchTest(this.listData, this.type);
  @override
