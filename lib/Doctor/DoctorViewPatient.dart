...
class DoctroViewPatent extends StatefulWidget {
  final snapshot;
  const DoctroViewPatent({Key key, this.snapshot}) : super(key: key);
  @override
  _ViewPatentState createState() => _ViewPatentState();
}

class _ViewPatentState extends State<DoctroViewPatent> {
  getData(id)async{
    String Id;
    ...
    http.Response rr = await http.post("http://www.notoutindia.com/aarogyam/api/TestSamples?DoctorId=$Id&PatientId=$id",
      headers: {
        "Authorization":prefs.getString('token'),
      },
    );
